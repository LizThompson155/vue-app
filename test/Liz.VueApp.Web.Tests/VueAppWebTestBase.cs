...
using Liz.VueApp.Models.TokenAuth;
...
{
    public abstract class VueAppWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static VueAppWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(VueAppWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<VueAppDbContext> action)
        {
            using (var context = IocManager.Resolve<VueAppDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<VueAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<VueAppDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<VueAppDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<VueAppDbContext, Task<T>> func)
        {
            T result;
